...
class EventBranch(Document):
	...
	def create_event_consumer(self):
		...
		if self.is_producer_online():
			...
			print("producer_siteeeeeeeeeeeeeeeeeeeeeeeeeee",producer_site) 
			...
			data = json.loads(json.dumps(self.get_request_data()))
			print("heloooooooooooooooooooooooooooooooooooooo",data)
			...
			frappe.msgprint("Event consumer Created")
        ...
        {"ref_doctype": entry.get("doctype"), "status": "Pending", "condition": entry.get("condition")},
			...
			consumer.insert
			frappe.msgprint("Event consumer Inserted")
	...
	def get_request_data(self):
		consumer_doctypes = []
		for entry in self.branch_doctypes:
